...
def train(args):
    ...
    model = KnowledgeEmbedding(dataset, args)

    if args.iteration > 1:
        initial_iteration = args.epochs + (args.retraining_epochs * (args.iteration - 2))
        num_epochs = args.retraining_epochs
        checkpoint = torch.load('{}/transe_model.ckpt'.format(args.log_dir), weights_only=False)
    ...
    review_to_train = len(dataset.review.data) * num_epochs + 1
    ...
    for epoch in range(initial_iteration+1, initial_iteration+num_epochs+1):
        dataloader.reset()
...
def extract_embeddings(args):
    ...
    epochs = args.epochs + (args.retraining_epochs * (args.iteration - 1))

    model_file = '{}/transe_model.ckpt'.format(args.log_dir)
...
def main():
    ...
    parser.add_argument('--retraining_epochs', type=int, default=3, help='retraining epochs')
