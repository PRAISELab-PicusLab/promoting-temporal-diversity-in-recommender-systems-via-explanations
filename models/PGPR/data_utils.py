...
class DataLoader(object):
    ...
    def get_batch(self):
        ...
        for pr in self.product_relations:
            data = getattr(self.dataset, pr).data
            if 0 <= product_idx < len(data):
                product_knowledge[pr] = data[product_idx]
            else:
                product_knowledge[pr] = None

        while len(batch) < self.batch_size:
            data = [user_idx, product_idx]
            for pr in self.product_relations:
                if product_knowledge[pr] is None or not product_knowledge[pr]:  # if len(product_knowledge[pr]) <= 0: OLD VERSION
                    data.append(-1)
                else:
