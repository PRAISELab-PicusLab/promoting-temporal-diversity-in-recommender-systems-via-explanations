...
LABEL_FILE = {
    ...
    CELLPHONES: (DATA_DIR[CELLPHONES] + '/train.txt.gz', DATA_DIR[CELLPHONES] + '/test.txt.gz'),
}

EMBED_FILE = {
    ML1M: TMP_DIR[ML1M] + '/train_transe_model/transe_model.ckpt',
    LFM1M: TMP_DIR[LFM1M] + '/train_transe_model/transe_model.ckpt',
    CELLPHONES: TMP_DIR[CELLPHONES] + '/train_transe_model/transe_model.ckpt',
}
...
def parse_args():
    ...
    parser.add_argument('--topk_candidates', type=int, default=100, help='weighting factor for ranking loss.')
    parser.add_argument('--iteration', type=int, default=1, help='iteration')
    parser.add_argument('--retraining_epochs', type=int, default=1, help='retraining epochs')

    # Hyperparameters for execute neural programs (inference).
    parser.add_argument('--sample_size', type=int, default=150, help='sample size for model.')
    ...
    args.symbolic_model = f'{args.log_dir}/symbolic_model.ckpt'
...
def load_embed_sd(dataset):
    state_dict = torch.load(EMBED_FILE[dataset], map_location=lambda storage, loc: storage, weights_only=False)
    return state_dict
